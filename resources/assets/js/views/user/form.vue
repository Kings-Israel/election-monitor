...
export default {
    ...
    watch: {
        search(val) {
            val &&
                val !== this.userForm.allocated_area &&
    ...
    methods: {
        ...
        submit(e) {
            ...
            axios
                .post('http://172.104.245.14/electionmonitor/api/v1/auth/register', this.userForm)
                // .post("/api/v1/auth/register", this.userForm)
                .then((response) => {
                    notify({
        ...
        getUsers() {
            this.loading = true;
            axios
                .get('http://172.104.245.14/electionmonitor/api/v1/user/'+this.id)
        ...
        updateUser() {
            ...
            this.userForm
                .put('http://172.104.245.14/electionmonitor/api/v1/user/'+this.id)
        ...
        getPollings() {
            this.loading = true;
            axios
                .get('http://172.104.245.14/electionmonitor/api/v1/polling')
                // .get("/api/v1/polling")
                .then((response) => {
                    this.loading = false;
                    for (let i = 10; i < response.data.length; i++) {
                        this.pollings.push(
                            response.data[i].polling_name +
                                "," +
                                response.data[i].county_name
                        );
                    }

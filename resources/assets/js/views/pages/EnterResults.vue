    ...
    <div class="container-fluid">
        ...
        <div class="card mb-4 mt-4">
            ...
            <div class="card-body px-3 pt-3 pb-2">
                <!-- Tab Nav -->
                <div
                    class="nav-wrapper position-relative mb-2"
                    v-if="$route.params.location === 'county'"
                >
                    <ul
                    ...
                    >
                        <li class="nav-item">
                            <a
                                class="nav-link mb-sm-3 mb-md-0 d-flex align-items-center justify-content-center"
                                id="tabs-icons-text-2-tab"
                                @click="filterAspirants('All')"
                                aria-controls="tabs-icons-text-2"
                                aria-selected="false"
                                style="background-color: #040539; color: #fff"
                            >
                        ...
                        <li class="nav-item">
                            <a
                                class="nav-link mb-sm-3 mb-md-0 d-flex align-items-center justify-content-center"
                                id="tabs-icons-text-2-tab"
                                @click="filterAspirants('County Governor')"
                                aria-controls="tabs-icons-text-2"
                                aria-selected="false"
                                style="background-color: #040539; color: #fff"
                            >
                ...
                >
                    <template v-slot:item.actions="{ item }">
                        <form
                            @submit.prevent="feedResult(item.uuid, item.id)"
                            :id="item.uuid"
                            v-if="!uuids.includes(item.uuid)"
                        >
                            <div class="row">
                                <div class="col-6">
                                    <input
                                        ...
                                        required
                                        v-model="
                                            resultsDetails.results[item.id]
                                        "
                                    />
                                </div>
                                <div class="col-6">
                        ...
                        <template v-else>
                            <p v-for="vote in voted" :key="vote.aspirant_uuid">
                                <span v-if="item.uuid === vote.aspirant_uuid"
                                    ><code>{{
                                        Number(vote.votes).toLocaleString()
                                    }}</code></span
                                >
                            </p>
                        </template>
...
export default {
    ...
    methods: {
        filterAspirants(electoral_position) {
            axios
                .get("http://172.104.245.14/electionmonitor/api/v1/enter-results/" +this.$route.params.electoral_area)
                // .get("/api/v1/enter-results/" +this.$route.params.electoral_area)
                .then((response) => {
        ...
        feedResult(uuid, index, id) {
            ...
            // var self = this
            this.resultsDetails
                .post("http://172.104.245.14/electionmonitor/api/v1/enter-results")
                // .post("/api/v1/enter-results")
                .then((response) => {
                    setTimeout(
                        () => (this.loading = false),
                    ...
                    axios
                        .get("http://172.104.245.14/electionmonitor/api/v1/vote-status")
                        ...
                        .then((response) => {
                            ...
                            var count_aspirants_voted = intersected.length;
                            this.progress =
                                (count_aspirants_voted / count_aspirants) * 100;
                            ...
                            if (this.$route.params.location === "county") {
                                axios
                                    .post("http://172.104.245.14/electionmonitor/api/v1/county_progress", { prog: Math.round(this.progress, 1) })
                                    // .post("/api/v1/county_progress", { prog: Math.round(this.progress, 1) })
                                    .then((response) => {
                                        // console.log(response)
                                    });
                            } else if (
                                this.$route.params.location === "national"
                            ) {
                                axios
                                    .post("http://172.104.245.14/electionmonitor/api/v1/national_progress",{ prog: Math.round(this.progress, 1) })
                                    // .post("/api/v1/national_progress",{ prog: Math.round(this.progress, 1) })
                                    .then((response) => {
                                        // console.log(response)
                                    });
                            } else if (
                                this.$route.params.location === "constituency"
                            ) {
                                axios
                                    .post("http://172.104.245.14/electionmonitor/api/v1/constituency_progress", { prog: Math.round(this.progress, 1) })
                                    // .post("/api/v1/constituency_progress", { prog: Math.round(this.progress, 1) })
                                    .then((response) => {
                                        // console.log(response)
                                    });
                            } else if (this.$route.params.location === "ward") {
                                axios
                                    .post("http://172.104.245.14/electionmonitor/api/v1/ward_progress", { prog: Math.round(this.progress, 1) })
                                    // .post("/api/v1/ward_progress", { prog: Math.round(this.progress, 1) })
                                    .then((response) => {
                                        // console.log(response)
                                    });
                            }
    ...
    mounted() {
        ...
        axios
            .get("http://172.104.245.14/electionmonitor/api/v1/vote-status")
            // .get("/electionmonitor/api/v1/vote-status")
            .then((response) => {
